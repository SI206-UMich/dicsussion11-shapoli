...
def add_fluffle(cur, conn):
    cur.execute('insert into Patients (pet_id, name, species_id, age, cuteness, aggressiveness) values(?,?,?,?,?,?)', (0,"Fluffley", 0, 3, 90, 100)) 
    conn.commit()
...
def add_pets_from_json(filename, cur, conn):
    ...
    for x in json_data:
        cur.execute('SELECT id FROM Species WHERE title =?', (x['species'],))
    conn.commit()
...
def non_aggressive_pets(aggressiveness, cur, conn):
    tup_lst = []
    cur.execute('SELECT name from Patients WHERE aggressiveness <= 10', tup_lst.append())
    conn.commit()
